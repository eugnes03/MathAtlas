...
(defn- attach-proofs
  ...
  (let [[acc pending]
        (reduce
          (fn [[acc pending] obj]
            (cond
              ;; proof: attach to pending provable and keep pending (don't emit yet)
              (= :proof (:type obj))
              (if pending
                [acc (assoc pending :proof-latex (:latex obj))]
                [(conj acc obj) nil])  ; orphan — keep as standalone

              ;; provable: flush previous pending, make this the new pending
              (provable-types (:type obj))
              [(cond-> acc pending (conj pending)) obj]

              ;; anything else: flush pending, emit this object
              :else
              [(cond-> acc pending (conj pending) true (conj obj)) nil]))
          [[] nil]
          objects)]
    ;; flush any trailing pending provable
